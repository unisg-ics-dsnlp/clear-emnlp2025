...
def create_heatmap(df, name, cell_width=70, cell_height=70):
    """
    Creates a heatmap using Matplotlib and saves it as a cropped PNG.

    Parameters:
        df (pd.DataFrame): The input DataFrame.
        name (str): Name of the output file.
        cell_width (int): Width of each heatmap cell in pixels.
        cell_height (int): Height of each heatmap cell in pixels.
        save_path (str): Directory where the image will be saved.
    """
    return
    # remove rows with index 'improved_length' and 'original_length'
    df = df.drop(['improved_length', 'original_length'], errors='ignore')

    row_count, col_count = df.shape  # Get number of rows and columns

    # Compute figure dimensions based on cell size
    fig_width = col_count * (cell_width / 100)  # Convert to inches (assuming 100 dpi)
    fig_height = row_count * (cell_height / 100)  # Convert to inches
    ...
    sns.heatmap(
        df,
        annot=False,  # Set to True if you want the values displayed inside the heatmap cells
        cmap='coolwarm',
    ...
    file_path = os.path.join(current_file_dir, 'heatmaps', f'{name}.png')
...
def general_analysis_table(
    ...
    for level in levels:
        ...
        for ds, r, ol in zip(datasets, relevant_percs, DATASET_LENS):
        ...
        if level == LinguisticLevel.SYNTACTIC:
            ...
            tmp_merge_all = []
            for r in relevant_percs:
                tmp = r[1][['americano_coherence_avgs', 'americano_persuasion_avgs', 'original_length', 'improved_length']]
                tmp.replace([np.inf, -np.inf], np.nan, inplace=True)
            ...
            for model_name in model_out_name:
                for approach in approach_filter:
                    for dataset in datasets:
                        relevant_files.append((model_name, approach, dataset, f'{model_name}_{dataset}__{approach}_comparison.csv'))
