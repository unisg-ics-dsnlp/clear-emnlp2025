...
class ArgumentDataFrame(pd.DataFrame):
    def __init__(
            self,
            df: pd.DataFrame,
    ):
        super().__init__(df)
        if not all(col in df.columns for col in ['topic_id', 'graph_id', 'stance', 'argument']):
            raise ValueError('Columns must be ["topic_id", "graph_id", "stance", "argument"]')
...
def bsm_formatter_feedback(
        topic: str,
        feedback: str,
        prompt: str
):
    return prompt.replace('>topic<', topic).replace('>FEEDBACK<', feedback)

def prepare_argument_improvement_prompt(
    ...
    for i, row in tqdm(argument_df.iterrows(), total=len(argument_df)):
        ...
        if use_feedback:
            system_prompt += f'\nIncorporate the following feedback to improve the argument:\n{row["feedback"]}'
...
def improve_arguments_direct(
    ...
    unformatted_prompts = prepare_argument_improvement_prompt(argument_df, demonstrations, use_feedback=use_feedback)
    direct = DirectPrompt(
        model,
        *args,
...
Aspects: >group<'''
    ...
    for i, row in tqdm(argument_df.iterrows(), total=len(argument_df)):
        ...
        if use_feedback:
            merge_formatters.append(partial(bsm_formatter_feedback, topic, row['feedback']))
        else:
...
def improve_arguments_selfdiscover(
        argument_df: ArgumentDataFrame,
        model: str,
        ...
        use_feedback: Optional[bool] = False
) -> tuple[list[str], list[str], tuple[str, str, str, str]]:
    """
    ...
    :param use_vllm: Whether to use VLLM.
    :param vllm_model_instance: The VLLM model instance.
    :param demonstration_args: The demonstration arguments.
    :return: Demonstration arguments, format should be (old, new).
    :return:
    """
    ...
    formatted_model_prompts = [selfdiscover.format_prompt(system_prompt, user_prompt, assistant_prompt) for
                               system_prompt, user_prompt, assistant_prompt in unformatted_prompts]
    outputs, (select_prompt, adapt_prompt, implement_prompt, execute_prompt) = selfdiscover.do_prompting(
        formatted_model_prompts,
        demonstrations,
    ...
    return improved_arguments, original_arguments, (select_prompt, adapt_prompt, implement_prompt, execute_prompt)
...
def improve_arguments_de(
    ...
    if dev_set_genetic is None:
        dev_set_genetic = get_demonstrations(50)
    # TODO: check this - it seems EXTREMELY slow
    ...
    for old, new in demonstration_args:
        demonstrations.append(f'Original: {old.strip()}\nImproved: @{new.strip()}@\n\n')
    unformatted_prompts = prepare_argument_improvement_prompt(argument_df, demonstrations)
    direct = DirectPrompt(model, format_func=format_func, use_vllm=use_vllm, vllm_model_instance=vllm_model_instance)
    # use optimized prompt to improve the arguments, instead of the system prompt from unformatted_prompts
    formatted_model_prompts = [direct.format_prompt(optimized_prompt, user_prompt, assistant_prompt) for
                               _, user_prompt, assistant_prompt in unformatted_prompts]
...
def improve_arguments_little_brother(
        argument_df: ArgumentDataFrame,
        model: str,
        use_vllm: bool,
        num_demonstrations: int = 3,
...
if __name__ == '__main__':
    ...
    improve_arguments_bsm(_df, 'llama3')
    ...
    improve_arguments_ga(_df, 'llama3', llama3_format_func, False)
    improve_arguments_de(_df, 'llama3', llama3_format_func, False)
    improve_arguments_little_brother(_df, 'llama3')
