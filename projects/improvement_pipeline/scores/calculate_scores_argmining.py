...
for dataset in DATASETS:
    ...
    for model in MODELS:
        ...
        if dataset == 'REVISIONS1':
            ...
            out_file_name = f'{model}_revision1__REVISIONS_FEEDBACK'
            ...
            out_file_name = f'{model}_revision2__REVISIONS_FEEDBACK'
            ...
            out_file_name = f'{model}_revision3__REVISIONS_FEEDBACK'
        else:
            file_path = os.path.join(current_file_dir, '..', 'improved_out', dataset, f'{model}.json')
            out_file_name = f'{model}__{dataset}'
        if os.path.exists(file_path):
            with open(file_path, 'r') as f:
                data = json.loads(f.read())
            for e in data:
                approach = e['approach']
                out_file_name_full = f'{out_file_name}__{approach}_comparison.csv'
