...
def get_cola_score(sentences):
    ...
    saved_pretrained_CoLA_model_dir = os.path.join(current_file_dir, 'cola_model', model_name)
    ...
    def compute_sentence_similarity():
        ...
        for i in range(len(all_summary)):
            ...
            for j in range(1, len(all_summary[i])):
                ...
                try:
                    hook = WMD.SpacySimilarityHook(doc1)
                    sim = hook.compute_similarity(doc1, doc2)
                    score.append(1.0 /
                                 (1.0 + math.exp(-sim + 7)))
                except:
                    score.append(1.0)
