...
from projects.text_generation_scores.americano_scores import calculate_americano_scores
...
from util.template import llama3_format_func, phi_format_func, mixtral_format_func, gemma_format_func, PromptTemplate
...
def do_all_scores_separately(
    ...
    do_americano_scores(
        ...
        improved_texts=improved_texts,
        out_identifier=out_identifier,
        model_name=americano_model_name,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--model-name',
        type=str,
        help='Model to use for AMERICANO scores',
        required=True
    )
    parser.add_argument(
        '--format-func',
    ...
    if not 'medium' in model_name_:
        raise ValueError('skipping this - ugly workaround but whatever')
    ...
    if model_name_ != 'llama3':
        print(f'> LOADING VLLM MODEL FOR AMERICANO SCORES - {model_name_}')
        model_instance = LLM(
            model=model_name_,
            dtype='half',
        ...
        print('> INITIALIZING DIRECTPROMPT FOR AMERICANO SCORES')
        direct = DirectPrompt(
            model=model_name_,
            temperature=0.7,
    ...
    for approach in data:
        ...
        do_all_scores_separately(
            ...
            improved,
            'feng_hirst_tmp',
            f'{approach_name}_{folder}_{out_name}',
            f'{out_name}__{folder}__{approach_name}',
            americano_model_name=model_name_,
            format_func=format_func_,
